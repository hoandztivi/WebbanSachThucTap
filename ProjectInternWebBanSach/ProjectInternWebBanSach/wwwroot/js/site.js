...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // ==== AVATAR PC DROPDOWN (CLICK) ====
    const userMenuPc = document.getElementById("userMenuPc");
    const userTogglePc = document.getElementById("userTogglePc");
    const userDropdownPc = document.getElementById("userDropdownPc");

    if (userMenuPc && userTogglePc && userDropdownPc) {

        userTogglePc.addEventListener("click", (e) => {
            e.stopPropagation();
            const open = userDropdownPc.classList.contains("opacity-100");

            if (open) {
                userDropdownPc.classList.add("opacity-0", "scale-95", "pointer-events-none", "hidden");
                userDropdownPc.classList.remove("opacity-100", "scale-100");
            } else {
                userDropdownPc.classList.remove("opacity-0", "scale-95", "pointer-events-none", "hidden");
                userDropdownPc.classList.add("opacity-100", "scale-100");
            }
        });

        document.addEventListener("click", (e) => {
            if (!userMenuPc.contains(e.target)) {
                userDropdownPc.classList.add("opacity-0", "scale-95", "pointer-events-none", "hidden");
                userDropdownPc.classList.remove("opacity-100", "scale-100");
            }
        });
    ...
    // ==== CLICK NGOÀI CATEGORY ====
    document.addEventListener("click", (e) => {
        if (categoryDropdown && categoryToggle && !categoryMenu.contains(e.target)) {
            categoryDropdown.classList.add("hidden");
        ...
        if (mobileDropdown && mobileToggle) {
            const li = mobileToggle.parentElement;
